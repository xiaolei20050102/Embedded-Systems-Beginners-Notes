...
## 关于定时中断 $CK\_CNT\_OV = CK\_PSC / (PSC + 1) / (ARR + 1)$

------

#### **1. `CK_PSC`**

- **是什么**：**定时器的输入时钟频率**（单位：Hz）。
- **来源**：**STM32的系统时钟（如72MHz）或外部时钟。**
- **例子**：如果系统时钟是72MHz，`CK_PSC = 72,000,000 Hz`。

#### **2. `PSC + 1`**

- **是什么**：**预分频系数（Prescaler），降低输入时钟频率**。
- **作用**：将高速的 `CK_PSC`分频为较低的计数器时钟 `CK_CNT`。
- **范围**：`PSC`是16位寄存器（0~65535），实际分频系数是 `PSC + 1`。
- **公式**：`CK_CNT = CK_PSC / (PSC + 1)`
    - **例子**：`PSC = 7199`→ 分频系数=7200 → `CK_CNT = 72MHz / 7200 = 10kHz`。

#### **3. `ARR + 1`**

- **是什么**：**自动重装载值（Auto-Reload Register），决定计数器的溢出周期**。
- **作用**：计数器从0开始累加，达到 `ARR`时溢出并触发中断。
- **范围**：`ARR`是16位寄存器（0~65535），实际计数次数是 `ARR + 1`。
- **公式**：`溢出频率 = CK_CNT / (ARR + 1)`
    - **例子**：`ARR = 9999`→ 计数10000次 → `溢出频率 = 10kHz / 10000 = 1Hz`（即1秒1次中断）。
...
好的，以下是为你整理的**STM32定时器中断流程笔记**，使用简洁的要点式记录，方便日后快速回顾：

------

#### **一、初始化配置流程（必须按顺序）**

1. **开启时钟** → `RCC_APBxPeriphClockCmd(...)`
    - 定时器挂载在APB1或APB2总线，需先使能其时钟。
2. **配置时基单元** → `TIM_TimeBaseInit(...)`
    - `PSC`(预分频器)：决定**计数频率**。`CK_CNT = CK_PSC / (PSC + 1)`
    - `ARR`(自动重载值)：决定**溢出周期**。计数器从0数到ARR后溢出。
    - `CounterMode`：通常为向上计数 `TIM_CounterMode_Up`。
3. **使能中断** → `TIM_ITConfig(..., TIM_IT_Update, ENABLE)`
    - 告诉定时器：溢出时**可以产生**中断请求。
4. **清除中断标志** → `TIM_ClearFlag(...)`或 `TIM_ClearITPendingBit(...)`
    - **关键步骤**：防止初始化后因标志位已置位而**立即进入中断**。
5. **配置NVIC（中断控制器）**
    - 设置中断通道 `TIMx_IRQn`。
    - 设置抢占优先级和子优先级。
    - 最后使能该中断通道。
6. **启动定时器** → `TIM_Cmd(..., ENABLE)`
    - **最后一步**：所有配置完成后，才让计数器开始计数。

------

#### **二、硬件自动运行流程**

1. **计数器递增**：启动后，CNT寄存器从0开始，每个CK_CNT周期+1。
2. **溢出触发**：当 `CNT = ARR`时，硬件自动：
    - 将状态寄存器（SR）中的**更新中断标志（UIF）** 置1。
    - 如果中断已使能（UIE=1），则向NVIC发送中断请求。
3. **NVIC响应**：NVIC根据优先级，调用对应的**中断服务函数（ISR）**。
...
```
void TIMx_IRQHandler(void) { // 函数名必须与启动文件一致
    if (TIM_GetITStatus(TIMx, TIM_IT_Update) == SET) { // 1. 检查中断源
        // 2. 在这里执行你的任务代码（如翻转LED、处理数据）
        TIM_ClearITPendingBit(TIMx, TIM_IT_Update); // 3. 【必须】清除中断标志！
    }
}
```

- **核心三要素**：检查、执行、清除。
- **不清除标志的后果**：中断会**无限循环触发**，程序卡死在ISR中。
...
- **中断频率公式**：
    ...
    `F_interrupt = F_CK_PSC / [(PSC + 1) * (ARR + 1)]`

    - 例如：72MHz时钟，PSC=7199，ARR=9999 → `72,000,000 / (7200 * 10000) = 1 Hz`(1秒1次)

- **PSC 和 ARR** 都是16位寄存器，取值范围：0 ~ 65535。
...
1. **不进中断**：检查NVIC配置、中断是否使能、IRQ函数名是否正确。
2. **只进一次中断**：肯定是在ISR中**忘了清除标志位**。
3. **中断太频繁**：计算PSC和ARR的值是否正确。
4. **初始化后立即进中断**：忘了在初始化后、启动前**清除标志位**。

------

**总结：配置按顺序（时钟→时基→使能中断→清标志→NVIC→启动），ISR三要素（检查→执行→清标志）。** 只要记住这个核心流程和排查点，就能快速上手和调试。
